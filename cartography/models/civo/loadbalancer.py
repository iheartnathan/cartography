...
class CivoLoadBalancerSchema(CartographyNodeSchema):
    ...
    `PROTECTED_BY`, `EXPOSES`, and `PART_OF_NETWORK` link the load balancer
    to its firewall, Kubernetes cluster, and network when those referenced
...
class CivoLoadBalancerBackendSchema(CartographyNodeSchema):
    ...
    `ROUTES_TO` links this backend to the `CivoInstance` it targets,
    matched by private IP scoped to the same network and account."""
