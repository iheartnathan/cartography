...
def test_civo_volumes_objectstores_graph(
    ...
    network_id relationship matchers are exercised here at the transform/
    property level only. Their edges require the separate Compute/
    Kubernetes/Networking PRs' node types to resolve against and can't be
    verified from this PR in isolation (see the PR split plan - unresolved
    matchers simply don't create edges until both domains are present in
    the same sync).
    """
    # Arrange
