...
def test_civo_instance_graph(mock_account_get, mock_instances_get, neo4j_session):
    """
    CivoInstance loaded standalone - network_id/firewall_id relationship
    matchers are exercised here at the transform/property level only. The
    PART_OF_NETWORK/PROTECTED_BY edges themselves require the separate
    Networking PR's CivoNetwork/CivoFirewall nodes to resolve against and
    can't be verified from this PR in isolation (see the PR split plan -
    unresolved matchers simply don't create edges until both domains are
    present in the same sync).
    """
    # Arrange
