...
def test_civo_loadbalancer_ip_graph(
    ...
    network_id/instance-backend relationship matchers are exercised here at
    the transform/property level only. Their edges (PROTECTED_BY, EXPOSES,
    PART_OF_NETWORK, ROUTES_TO, and the instance half of ASSIGNED_TO)
    require the separate Networking/Compute/Kubernetes PRs' node types to
    resolve against and can't be verified from this PR in isolation (see
    the PR split plan).
    """
    # Arrange
